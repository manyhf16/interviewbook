...
以其中一段为例：
```
"http-bio-9090-exec-2" #21 daemon prio=5 os_prio=0 tid=0x000000005a058000 nid=0x1d5c waiting on condition 
```
它的含义是：有一个线程名为 `http-bio-9090-exec-2` 从名字上可以知道它是一个tomcat中以block-io方式工作线程，线程类型是`守护线程`，线程id 是`0x1d5c`(16进制)
这个线程id可以配合 ps 命令(linux)或pslist 命令(windows) 来进一步调试
